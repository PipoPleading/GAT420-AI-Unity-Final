        ...
        while (!found && nodes.Count > 0) 
        {
            ...
            foreach (var neighbor in node.neighbors)
            {
                ...
                if(cost < neighbor.Cost)
                {
                    ...
                    float bonusHeuristic = Vector3.Distance(neighbor.transform.position, endNode.transform.position);
                    nodes.EnqueueWithoutDuplicates(neighbor, neighbor.Cost + bonusHeuristic);
                }
            }
        }
